...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.dir.is_none() && args.profile.is_none() {
        ...
        run_profile(&home_dir, profile_path.as_path(), &validator_info)?;
        ...
        if let Some(profile_name) = args.profile {
            ...
            if profile_path.exists() {
                ...
                run_profile(&home_dir, &profile_path, &validator_info)?; // Run commands
            ...
            for entry in profiles {
                ...
                if path.extension().and_then(|s| s.to_str()) == Some("privkey") {
                    ...
                    run_profile(&home_dir, &path, &validator_info)?; // Run commands
