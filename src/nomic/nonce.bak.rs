...
/// - `Err(eyre::Report)` if there is an issue retrieving the nonce file path.
...
/// - `Err(eyre::Report)` if an error occurs while retrieving the nonce file or reading its contents.
...
/// - `Err(eyre::Report)` if an error occurs while retrieving the nonce file path or writing its contents.
